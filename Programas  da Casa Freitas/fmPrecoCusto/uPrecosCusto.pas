...
uses
  ...
  TFlatCheckBoxUnit, fCtrls, uGetNotas;
...
type
  TfmPrecoCustos = class(TForm)
    ...
    Button2: TButton;
    btRecalculaCMUItensNota: TButton;
     ...
     SCREEN.Cursor := crHourGlass;
      ...
      begin
         ...
         query.sql.add( ' select is_uo, ds_uo as Loja, ' +
                        ' dbo.Z_CF_funObterPrecoProduto_CF(1, ' +cod+'  , is_uo , 10033585) VlCusto, ' +
                        ' dbo.Z_CF_funObterPrecoProduto_CF(5, ' +cod+'  , is_uo , 10033585) VlCMU, ' +
                        ' dbo.Z_CF_funObterPrecoProduto_CF(103, ' +cod+'  , is_uo , 10033585) VlAtacado, ' +
                        ' dbo.Z_CF_funObterPrecoProduto_CF(103, ' +cod+'  , is_uo , 10033585) * .07  as [70% de VlAtacado], ' +
                        ...
                        if cbLoja.itemIndex > 0  then
                           query.sql.add( ' and is_uo in( 10033674, ' +  funcoes.getCodUO(cbLoja)  +')' )
                        else
...
procedure TfmPrecoCustos.ConnectionWillExecute(Connection: TADOConnection;  var CommandText: WideString; var CursorType: TCursorType;  var LockType: TADOLockType; var CommandType: TCommandType;  var ExecuteOptions: TExecuteOptions; var EventStatus: TEventStatus;  const Command: _Command; const Recordset: _Recordset);
begin
   screen.Cursor := crHourGlass;
   funcoes.GravaLinhaEmUmArquivo(funcoes.TempDir() + '_Log_'+ application.Title+'.txt', CommandText + #13)
end;

procedure TfmPrecoCustos.ConnectionExecuteComplete(Connection: TADOConnection;  RecordsAffected: Integer; const Error: Error;  var EventStatus: TEventStatus; const Command: _Command;  const Recordset: _Recordset);
begin
   screen.cursor := crDefault;
end;
...
var
  ...
  cmd: String;
begin
   cmd := ' select p.cd_ref, dbo.z_cf_funObterprecoproduto_CF( 6, p.is_ref, 0 ,0) as valor ' + 
   ...
   begin
      ...
      edPcNovo.Value := ds.fieldByName('valor').AsFloat;
      btAjustaPrecoClick(nil);
      mmResult.Lines.Add('cod: ' +  ds.fieldByName('cd_ref').AsString +' valor: ' + ds.fieldByName('valor').AsString );
      ds.Next();
   end;
end;
...
procedure TfmPrecoCustos.Button2Click(Sender: TObject);
begin
   efetuaAjustDeNota('1');
end;
...
//   cbLoja.Items :=  funcsQL.getNomeLojas(  fmMain.Conexao, true , false,'','' );
