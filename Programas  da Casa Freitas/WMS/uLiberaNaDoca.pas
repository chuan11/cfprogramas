...
procedure TfmRecebeNota.fsBitBtn1Click(Sender: TObject);
begin
...
begin
   ...
   SoftDBGrid1.Columns[ ds.FieldByName('int_numero_nfe').Index ].Visible := false;
   SoftDBGrid1.Columns[ ds.FieldByName('str_serie_nfe').Index ].Visible := false;

   if ( funcoes.ExisteParametro('-wms') = true) then
   begin
      SoftDBGrid1.Columns[ ds.FieldByName('data Ent').Index ].Width := 65;
      SoftDBGrid1.Columns[ ds.FieldByName('nota').Index ].Width := 50;
