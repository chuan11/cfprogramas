...
begin
   ...
   if ( edCEP.Text= '' ) or
      ( length(funcoes.SohNumeros(edCEP.text)) <> Length(edCEP.Text) ) or
   ...
   begin
      ...
      funcoes.msgTela('', erro, MB_OK + MB_ICONERROR);
   end;
...
var
  cod, desc:String;
begin
   if (pegarDadosCampo('cidade', 'cd_cid', 'nm_cid', edUF.Text, edUF.Text, cod, desc ) = true) then
   begin
      edCidade.Text := desc;
...
begin
   ...
   begin
      ...
      funcoes.msgTela('', 'CEP não cadastrado', MB_OK + MB_ICONERROR);
   ds.Free();
end;
...
begin
   if (OPERACAO = btIncluir.Caption) then
      ...
      begin
         ...
         begin
            funcoes.msgTela('', 'CEP incluido', MB_OK + MB_ICONEXCLAMATION);
            btCancelaClick(nil);
         end
      end
      else
         if (funcoes.msgTela('',' CEP já cadastrado, deseja consultar?', MB_YESNO + MB_ICONERROR) = mrYes ) then
            consultaCEP();
end;
...
begin
   if (funcoes.msgTela('','Eu recomento fortemente vc não excluir algum CEP, pois isso pode '+
                          ' causar inconsistência.' +#13+ 'Deseja realmente exluir o CEP ?', MB_YESNO + MB_ICONERROR ) = mrYes) then
   begin
      if (excluirCEP() = true) then
         funcoes.msgTela('', 'CEP excluido', MB_OK + MB_ICONEXCLAMATION)
      else
         funcoes.msgTela('', 'Erro ao Excluir CEP', MB_OK + MB_ICONEXCLAMATION);
      btCancelaClick(nil);
   end;
end;
...
begin
   if (funcoes.msgTela('','Eu recomento fortemente vc não alterar algum CEP, pois isso pode '+
                          ' causar inconsistência.' +#13+ 'Deseja realmente alterar o CEP ?', MB_YESNO + MB_ICONERROR ) = mrYes) then
      if (alteraCEP() = true) then
         msgTela('', 'Alteração concluida.', MB_OK + MB_ICONEXCLAMATION)
      else
         msgTela('', 'Erro ao tentar excluir.', MB_OK + MB_ICONEXCLAMATION);
end;
