...
uses
  ...
  ImgList, UAlteraForPedido, Grids, DBGrids, SoftDBGrid, Typinfo,
...
begin
   cmd :=
   ' Select distinct codTela, isAcessoRestrito, isPedeAutorizacao' +
   ' from zcf_telasPermitidas where grupo = ' + grupo +
   ' and codTela in ( select tag from zcf_telas)'+
   ' order by codTela';
   DS_PERMISSOES := funcSQl.getDataSetQ(cmd, conexao);
end;
...
begin
   ...
   begin
      for i:=0 to item.Count -1 do
         verificaPermissao(item.Items[i]);
    end
end;
...
var
   str:String;
begin
   funcoes.msgTela('', 'Erro:'+#13+e.Message, 0);
...
begin
   ...
   begin
     try
        ...
        dirRemoto :=
        fmMain.GetParamBD('ServerNFE.DirNFE', fmMain.getUoLogada() )+
        '\'+funcDatas.getAnoMes(ds.fieldByName('dt_emis').asString)+'\';

        dirLocal :=  funcoes.getDirLogs();
...
begin
   ...
   begin
      ...
      begin
         ...
         msgEmail.add( 'Emitida pela loja: '+ ds.FieldByName('loja').asString );
         enviarEmail( '', 'Envio de PDF, nota fiscal eletrônica', arqPDF,  msgEmail, 'Envio de XML');
         ds.Free();
         deleteFile(isNota);
      end;
   end;
...
function TfmMain.getSubPastaXML(strDataEmissao:String): String;
begin

    if (fmMain.getParamBD('ServerNFE.XMLPorMes','0') <> '0') then
       strDataEmissao := funcDatas.getAnoMes(strDataEmissao)+'\'
    else
       strDataEmissao := '';

   funcoes.gravaLog('getSubPastaXML():' + strDataEmissao);
...
begin
   ...
   begin
      try
         ...
         dirRemoto :=
         fmMain.GetParamBD('ServerNFE.DirNFE', fmMain.getUoLogada()) + fmMain.getSubPastaXML(ds.fieldByName('dt_emis').asString);
      ...
      if (impressora <> '') then
         ...
         begin
            ...
            dirNFE := funcSQL.getParamBD('ServerNFE.DirNFE', ds.FieldByName('is_estoque').AsString, fmMain.conexao)+
                      fmMain.getSubPastaXML(ds.fieldByName('dt_emis').asString);
