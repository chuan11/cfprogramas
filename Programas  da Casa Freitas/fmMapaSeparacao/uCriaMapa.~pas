...
type
  TfmCriarMapa = class(TForm)
    ...
    procedure insereItensMapa(Sender:Tobject;nAvaria:String);
...
procedure TfmCriarMapa.insereItensMapa(Sender: Tobject; nAvaria: String);
var
   ...
   begin
      ...
      begin
         cmd := ' Insert zcf_mapaSeparacaoI( num, is_ref, estI, saldo, pco, caixa ) '+
                ...
                ' where dsipe.is_pedf = ' + nAvaria  +  ' Order by dsipe.is_ref' ;
      end
      else if rg.ItemIndex = 2 then
      begin
          cmd := ' Insert zcf_mapaSeparacaoI( num, is_ref, estI, saldo, pco, caixa ) '+
                 ' Select '+ nAvaria+ ' , dmovi.is_ref, dmovi.qt_mov, dmovi.qt_mov,  '+
      ...
      if (funcsql.execSQL(cmd, fmMain.Conexao) = true ) then
      begin
          MsgTela('','Criada o mapa de número: ' + nAvaria+ #13+ 'O mapa será carregado.', MB_ICONEXCLAMATION + MB_OK  );
          fmMapa.CarregaMapa( nAvaria);
          BitBtn3.Click;
      end;
   end
   else
   begin
      funcSQl.execSQL('delete from zcf_mapaSeparacao where num = ' +nAvaria , fmMain.Conexao);
...
begin
   ...
   begin
      try
         ...
         nMapa := criarMapa(nil);
         if nMapa  <> '' then
               insereItensMapa( nil, nMapa );
      except
         on e:Exception do
         begin
            if pos('PK__zcf_mapaseparaca', e.Message) > 0 then
            msgTela('','Já existe um mapa de separação com esses dados.', MB_ICONERROR + MB_OK);
         end;
      end;
   end
   else
