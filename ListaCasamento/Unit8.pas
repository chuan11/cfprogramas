...
begin
   ...
   if (dt1.Date >= now)  or  (dt2.Date >= now) then
      MsgTela('','Voce só pode consultar listas que os eventos em que a data seja menor que a data de hoje. ', MB_ICONERROR + mb_ok)
    else
    begin
...
begin
   ...
   begin
      ...
      if (erroCad <> '') then
         msgTela('',' O codigo de cliente informado para well é inválido, '+#13,mb_ok + MB_ICONERROR);
   ...
   begin
      ...
      msgTela('','   Para imprimir os comprovantes de crédito, acesse a funcionalidade do well: Consulta ao crédito.', MB_OK + MB_ICONEXCLAMATION);
      BitBtn2Click(nil);
   end;
end;
...
var
  usr:String;
begin
   msgTela('','Para gerar o crédito, é preciso: ' + #13+
              ' - Que o código do cliente no well seja informado no cadastro da lista ' +#13+
              ' - Que o evento já tenha ocorrido ' + #13+
              'Após a geração do crédito a lista não poderá mais ser alterada.'  + #13+
              'Listas que já tenham gerado crédio não geram novamente.'  + #13
...
begin
   ...
   begin
      ...
      MsgTela('','Só é gerado o recibo de bonus após a geração do crédito', MB_OK + MB_ICONERROR);
end;
