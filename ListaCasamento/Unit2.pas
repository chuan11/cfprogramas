...
var
   ds:TdataSet;
begin
   ds:= funcSQL.getADODataSetQ( 'Select numLista from produtos where numLista = '+ NUMERO_LISTA, form1.ADOConnection1);
   if (ds.RecordCount < 5) then
   begin
      ds:= funcSQL.getADODataSetQ( 'exec stoListaSugestao '+ TIPO_LISTA, form1.ADOConnection1);
      ...
      funcoes.msgTela('', 'Os itens da lista já foram incluídos' +#13+
                          'Se quiser editar a lista, vc pode abrir a lista e editá-la.', MB_ICONERROR + MB_OK);
end;
...
begin
   if form1.MsgTela('A lista foi incuida.'+ #13+' Gostaria de incluir os itens agora??' +#13+ 'Se escolher não poderá inclui-ilos posteriormente' ,mb_iconquestion+mb_Yesno) = mrYes then
...
begin
   ...
   if (comandos.Count  > 0) then
      for i:=0 to comandos.Count -1 do
         funcSQL.execSQL( comandos[i], form1.ADOConnection1);

   form1.atualizaDadosItensLista( inttostr(lb2.items.count),'0','0', NUMERO_LISTA);
...
begin
   NUMERO_LISTA := form1.Query1.fieldByName('numLista').asString;
   TIPO_LISTA := form1.Query1.fieldByName('tipo').asString;
   abreJanelaInclusaoItens();
end;
