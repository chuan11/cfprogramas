...
begin
   // verifica se o cfo da nota é um dos cfos da String
...
// retirar os campos 'nf1'
    ...
    listbox1.items.add ('Alteração do CPF Feito!');
    form1.Refresh
end;
//*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*---*-*-*-*-*-
//*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*---*-*-*-*-*-
...
begin
    ...
    if selar = false then
    begin
       showmessage('O arquivo Já está Selado para NFVC.');
    end
    else
    begin
       reset(arq);
...
begin
   logar('Inicio Selar NF1 blocos.' );
    ...
    if checkbox1.Checked = false  then
       ...
       begin
          showmessage('A numeração está errada, verifique.')
       end
    else
    begin
       reset(arq);
       ...
       begin
          readln(arq,linha);
          if (copy(linha,1,6) = '400301') then // 4003 é ECF
          begin
...
{9900}    end
       ...
       DeleteFile(PATH+'SISIF2.txt');
       LimparCampos(sender);
       listbox1.items.add('Selado NF/MR de ' + edit1.text + ' até ' + edit3.text);
       form1.Refresh;
...
begin
   ...
   opendialog1.title := 'Selecione o arquivo das numerações de selos dos formularios.';
   if opendialog1.Execute then
   begin
...
begin
   ...
   logar('Variável DifFormulario: ' + intToStr(DifForm));
...
// retirar o fomulario que o well grava no arquivo
   ...
   screen.Cursor := crdefault;
   LimparCampos(sender);
   listbox1.items.add('Selado Formularios de ' + edit1.text + ' até ' + edit3.text);
   form1.Refresh;
end;
...
begin
   ...
   form1.openDialog1.Title :=' Selecione o arquivo da AIDF correspondente à loja.';
...
begin
   ...
   DeleteFile(PATH+'SISIF2.txt');
   LimparCampos(sender);
   listbox1.items.add('Ajuste de redução.');
   form1.Refresh;
end;
