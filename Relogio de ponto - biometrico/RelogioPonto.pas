...
type
  TfmMain = class(TForm)
    ...
    function isAntecipacaoSaida(matricula, data, batida:String):integer;
    ...
    procedure calcHorasTotais(var table: TADOTable; var tHorasPrev, tHorasTrab , tAtrasJus, tAtrasAut, tHorasDif, batIncJ, batInc , falta, faltaJ:smallint; dadosEmp:string; dataInicial:Tdate);
    ...
    procedure PreencherListaBatidas(var table:TadoTable; dti,dtf:Tdate ; dadosEmp:String);
...
function TfmMain.calculaAtraso(cartaoPonto:String; data:Tdate;batida:String):integer;
var
  aux:integer;
begin
...
procedure TfmMain.calcHorasTotais(var table: TADOTable; var tHorasPrev, tHorasTrab , tAtrasJus, tAtrasAut, tHorasDif, batIncJ, batInc , falta, faltaJ:smallint; dadosEmp:string; dataInicial: Tdate);
var
   intervalo,horasDia,difDia:integer;
   dataAux:Tdate;
   msgJust,msgObs,codJust, jusUsuario:String;
   PrevEnt,PrevSai:integer;
   matricula, cartaoPonto:string;
   strEstaAtivo:String;
begin
   cartaoPonto := getCartaoPontoStr(dadosEmp);
...
//  verificar os feriados no periodo e calcular o tempo previsto
   ...
   begin
      ...
      begin
         ...
         PrevSai := horaToInt( HoraPrev(dateToStr(dataAux), cartaoPonto, true, false ) ) ;

         table.FieldByName('ehFeriado').asString := 'N';

         intervalo :=  horaToInt( horaPrev( dateToStr(dataAux), cartaoPonto, true, true ) ) - horaToInt( horaPrev( dateToStr(dataAux), cartaoPonto, false, true ) );
...
// calcular horas trabalhadas diferenca
   ...
   begin
      ...
      begin
         ...
         begin
            intervalo := intervalo + horasDia - table.fieldByName('tprevisto').Asinteger;
            table.fieldByName('sinal').AsString := '+';
            table.fieldByName('dif').AsString :=  intToHora( horasDia - table.fieldByName('tprevisto').Asinteger);
         end
         else
         begin
            intervalo := intervalo - ( table.fieldByName('tprevisto').Asinteger - horasDia );
            table.fieldByName('sinal').AsString := '-';
...
procedure TfmMain.PreencherListaBatidas(var table:TadoTable; dti,dtf:Tdate ; dadosEmp:String);
var
...
begin
    ...
    batPeriodo := uUtil.getBatidas( dateToStr(dti), dateToStr(dtf), getCartaoPontoStr(dadosEmp) );
    i:=0;
    table.First;
    ...
    begin
       ...
       begin
          table.Edit;
          table.FieldByName('ent').asString := copy(bat[0],01,05);
          table.FieldByName('int_sai').asString := copy(bat[1],01,05);
          table.FieldByName('int_ent').asString := copy(bat[2],01,05);
          table.FieldByName('sai').asString := copy(bat[3],01,05);
          table.FieldByName('localEnt').asString := copy(bat[0],06,03);
          table.FieldByName('LocalSai').asString := copy(bat[3],06,03);
          table.post;
       end;
       inc(i);
       table.Next;
