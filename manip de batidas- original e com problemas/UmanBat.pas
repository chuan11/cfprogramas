...
uses uHorarios, uImportaCadastro, uCadEmpregados;

{$R *.DFM}


function  TfmMain.ExisteJustificativa(sender:Tobject;linha:string):boolean;
begin
...
function TfmMain.FormatoDahora(x:integer):String;
var
   aux,h, m :String;
begin
   h := inttoStr( x div 60 );
...
function TfmMain.Atraso(batida,data,horarios:String):String;
var
  Adate: Tdatetime;
...
procedure tfmMain.ajustaTelaParaAjusteFinos(sender:tobject;NomeArquivo:string);
begin
   fmMain.left:=0;
...
procedure  tfmMain.MsgDeRodape(msg:string);
begin
   fmMain.Refresh;
   fmMain.StatusBar1.simpletext := msg;
end;

function tfmMain.carregaCbox(str:string; cbox:TComboBox):integer;
var
  i,j:integer;
begin
   j := -1 ;
   for i:=0 to CBox.Items.count do
      if pos(str,CBox.Items[i]) > 0 then
         j:=i;
   if j = -1 then
      fmMain.msgDeRodape('Nao Cadastrado !!!!!!')
   else
      fmMain.msgDeRodape(copy(CBox.Items[j],01,25));
   carregaCbox := j;
end;
...
function TfmMain.horaInt(hora:String):String;
begin
   if hora <> '' then
...
function tfmMain.inserirBatidasDiaNormal(sender:TObject;linha:string):string;
var
   r:integer;
begin
 if fmMain.ExisteJustificativa(sender,linha) = false then
 begin
   r:= random(20);
   delete(linha, 51, 5);
...
function tfmMain.r1(n:string):string;
var
   aux:string;
begin
   aux:= copy(n,7,4);          //ano
...
function tfmMain.r2(m:string):string;
var
   aux:string;
begin
   aux:= copy(m,7,4);
...
function tfmMain.NomedoDia(diasem:string):string;
var
  ADate: TDateTime;
...
procedure tfmMain.CarregaListaEmpregados(sender: Tobject; lb:TComboBox);
var
   arq: TextFile;
   linha:String;
   i:integer;
...
procedure TfmMain.MaskEdit1Click(Sender: TObject);
begin
//   tabsheet2.setfocus;
end;
...
procedure tfmMain.OrganizaPorNome(sg2:TStringGrid);
var
...
procedure TfmMain.sg2SelectCell(Sender: TObject; ACol, ARow: Integer;
...
{
procedure tfmMain.impPontoPorDia(sender:TObject);
var
   arq:TExtFIle;
   i,j:integer;
begin
...
procedure TfmMain.BitBtn12Click(Sender: TObject);
begin
...
begin
   ...
   AnimateWindow(fmMain.Handle,150, AW_ver_POSITIVE);
   fmMain.Top := 01;
   fmMain.Left := 100;
   ...
   fmMain.conexaoWell.ConnectionString := funcoes.getDadosConexaoUDL( ExtractFilePath(ParamStr(0)) + 'ConexaoAoWell.ini');
   fmMain.conexaoWell.Open
end;
...
procedure tfmMain.procurarmemo2(sender:tobject);
begin
...
procedure TfmMain.TabSheet4Exit(Sender: TObject);
begin
...
procedure TfmMain.TabSheet4Enter(Sender: TObject);
begin
...
procedure tfmMain.salvarMemo2(sender:tobject);
begin
...
procedure TfmMain.AjustaRelatoriodePonto(Sender: TObject);
var
   arq, arq2: textFile;
   linha : string;
begin
...
procedure TfmMain.organizerpornome1Click(Sender: TObject);
begin
   showmessage('');
end;

procedure TfmMain.qq1Click(Sender: TObject);
begin
showmessage('');
end;

{
procedure tfmMain.selecionarEmpregados(base:String);
var
  ds:TDataSet;
begin
...
procedure TfmMain.Memo2KeyPress(Sender: TObject; var Key: Char);
begin
   KEY :=   UPCASE(KEY);
end;
...
procedure TfmMain.AjustaHorFixoTodos(base:string);
var
   cmd:String;
begin
   ConectarAoBd(base);
...
procedure tfmMain.AjustaCadastro(sender:Tobject);
var
...
function TfmMain.getNumMatricula(nCartao:String): String;
begin
...
function TfmMain.ProcuraEmCbox(sender: TObject; Str: string; Itens: Tstrings): integer;
var
  j,i:integer;
begin
   j:= -1;
   for i:= 0 to itens.count -1 do
      if pos(str,itens[i]) > 0 then
         j := i;
      if j = -1 then
         fmMain.msgDeRodape('Não Cadastrado !!!!')
      else
         result := j;
end;

function TfmMain.CarregaTabelaDeCadastro(Sender: Tobject): Boolean;
var
  Conexao:TADOConnection;
  Query:TADOQuery;
  Arq:TstringList;
...
procedure TfmMain.ConectarAoBd(nBase: String);
var
  bd:String;
begin
...
procedure TfmMain.conexaoWellExecuteComplete(Connection: TADOConnection;
  RecordsAffected: Integer; const Error: Error;
  var EventStatus: TEventStatus; const Command: _Command;
  const Recordset: _Recordset);
begin
...
procedure TfmMain.Cadastrodehorarios1Click(Sender: TObject);
begin
...
procedure TfmMain.CadastrodeEmpregados1Click(Sender: TObject);
begin
    PageControl1.Visible := false;
...
procedure TfmMain.Cadastrodeempregados2Click(Sender: TObject);
begin
