...
begin
//   PortaImpressao := fmMain.CBox2.Items[fmMain.cbox2.itemIndex];
//   cb1.Items := fmMain.cbox1.Items;
...
// Na primeira coluna coloca o dia a dia
   ...
   begin
       ...
       SG1.cells[0,i+1]:= DateToStr(dti.date + i)  + ' - ' +  fmMain.NomeDoDia( DateToStr(dti.date + i) );
...
begin
   ...
   if erro <> '' then
   begin
      application.MessageBox(pchar(erro), pchar(fmMain.Caption), mb_iconError + mb_OK);
   end
   else
   begin
...
//   writeln(origem, 'cmd /c print /d:' + fmMain.Cbox2.Items[fmMain.Cbox2.ItemIndex] +' '+ ExtractFilePath(ParamStr(0))+ 'Impressao.txt' );
   closeFile(Origem);
...
begin
  ...
  begin
      application.MessageBox(pchar('Ao menos uma ocorrência deve ser verificada'), pchar(fmMain.Caption), mb_iconError + mb_OK);
...
begin
   if memo1.Lines.Count -1 > 1 then
      if application.MessageBox(' Deseja imprimir essas ocorrência ? ', pchar(fmMain.caption), mb_YesNo + Mb_Iconquestion) = mrYes then
