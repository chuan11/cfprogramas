...
{$IFDEF LINUX}
     QStdCtrls, QControls, QTypes, QMask,
{$ELSE}
     StdCtrls, Controls, Messages, Mask,
{$ENDIF}
     adBoundLabel;

type

  //-----| Class: TadLabelMaskEdit

   TadLabelMaskEdit = class(TMaskEdit)
  private
...
constructor TadLabelMaskEdit.Create(AOwner: TComponent); 
begin
...
destructor TadLabelMaskEdit.Destroy;
begin
  FreeAndNil(FColors);
...
{$IFNDEF LINUX}
procedure TadLabelMaskEdit.CMBidimodechanged(var Message: TMessage);
begin
  inherited;
...
// -----------------------------------------------------------------------------
{$IFDEF LINUX}
procedure TadLabelMaskEdit.Visiblechanged;
{$ELSE}
procedure TadLabelMaskEdit.CMVisiblechanged(var Message: TMessage);
{$ENDIF}
begin
  inherited;
  if Assigned(FLabelDefs) then
...
{$IFDEF LINUX}
procedure TadLabelMaskEdit.EnabledChanged;
{$ELSE}
procedure TadLabelMaskEdit.CMEnabledchanged(var Message: TMessage);
{$ENDIF}
begin
  inherited;
  if Assigned(FLabelDefs) then
...
// -----------------------------------------------------------------------------
{$IFDEF LINUX}
procedure TadLabelMaskEdit.MouseEnter(AControl: TControl);
{$ELSE}
procedure TadLabelMaskEdit.CMMouseEnter(var Message: TMessage);
{$ENDIF}
begin
  inherited;
  DoEnter;
...
{$IFDEF LINUX}
procedure TadLabelMaskEdit.MouseLeave(AControl: TControl);
{$ELSE}
procedure TadLabelMaskEdit.CMMouseLeave(var Message: TMessage);
{$ENDIF}
begin
  inherited;
  DoExit;
...
procedure TadLabelMaskEdit.DoEnter;
begin
  inherited;
...
procedure TadLabelMaskEdit.DoExit;
begin
  inherited;
...
procedure TadLabelMaskEdit.SetColors(const Value : TadColors);
begin
...
procedure TadLabelMaskEdit.Notification(AComponent: TComponent; Operation: TOperation);
begin
...
procedure TadLabelMaskEdit.SetBounds(ALeft, ATop, AWidth, AHeight: Integer);
begin
...
procedure TadLabelMaskEdit.SetPosition(const Value: TadControlPosition);
begin
  if Assigned(FLabelDefs) then
...
procedure TadLabelMaskEdit.SetSpacing(const Value: Integer);
begin
  FLabelSpacing := Value;
  SetPosition(FLabelPosition);
end;
...
procedure TadLabelMaskEdit.SetName(const Value: TComponentName);
begin
...
{$IFDEF LINUX}
procedure TadLabelMaskEdit.SetParent(const Value: TWidgetControl);
begin
...
{$ELSE}
procedure TadLabelMaskEdit.SetParent(AParent: TWinControl);
begin
