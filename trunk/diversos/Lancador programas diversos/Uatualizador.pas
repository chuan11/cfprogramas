...
type
  TForm1 = class(TForm)
    ...
    procedure ExecutarAplicativo(NomeAplicativo:string);
    procedure FormActivate(Sender: TObject);
    procedure Atualizar(Sender:Tobject);
    procedure desconectarDirRemoto();
  private
...
begin
   ...
   if ( arq <> 'Upgrade.exe') then
   begin
      if (FileExists(PATH + arq) = false) or ( DataDeCriacao( PATH_ORIGEM + ARQ ) <> DataDeCriacao( PATH + arq )  ) then
         PodeBaixarArq := true
   end
   else
      if (FileExists(PATH + 'Upgrade.exe') = true ) and ( DataDeCriacao( PATH + 'Upgrade.exe'  ) <> DataDeCriacao( PATH_ORIGEM + 'UpgradeNovo.exe' )  ) then
         PodeBaixarArq := true;
end;
...
begin
  ...
  if (FileExists('upgrade.ini') = false ) then
  begin
     funcoes.GravaLinhaEmUmArquivo('upgrade.ini','00Dir atualizacao(sem "\" no fim) =\\125.0.0.250\PACKAGE\ProgramasDiversos' );
     funcoes.GravaLinhaEmUmArquivo('upgrade.ini','01Usa autorizaca Acessar o dir atualizacao?=1' );
     funcoes.GravaLinhaEmUmArquivo('upgrade.ini','02Usuario=convidado2' );
     funcoes.GravaLinhaEmUmArquivo('upgrade.ini','03senha=convidado2' );
     funcoes.GravaLinhaEmUmArquivo('upgrade.ini','03senha=convidado2' );
     ShowMessage('Carregados parametros default, configure os dados de conexão e execute novamente');
...
begin
   ...
   begin
    ...
    funcoes.GravaLinhaEmUmArquivo(funcoes.tempdir()+'AcertosAcertaHora.txt','programa upgrade, tentativa de acertar hora Acerto da hora '+  hora +' '+ min );
end;
...
begin
   ...
   ExecutarAplicativo( ProgramaAExecutar );
   Application.Terminate;
end;
...
procedure TForm1.ExecutarAplicativo(NomeAplicativo: string);
var
   cmd:String;
begin
   ...
   if (NomeAplicativo <> '') then
   begin
      cmd := 'cmd /c start '+ NomeAplicativo + ' ***';
...
procedure TForm1.desconectarDirRemoto;
var
   cmd:String;
begin
...
begin
   try
      PATH_ORIGEM :=   funcoes.lerParam('upgrade.ini',0);

      if PATH_ORIGEM <> '' then
      begin
         usuarioRemoto := funcoes.lerParam('upgrade.ini',02);
         if usuarioRemoto = '' then
            usuarioRemoto := 'convidado2';

         senha := funcoes.lerParam('upgrade.ini',03);
         if senha = '' then
            senha := 'convidado2';

         cmd := 'CMD.EXE /c net use '+PATH_ORIGEM+ ' /USER:'+ usuarioRemoto +' '+senha;

         desconectarDirRemoto();
         Winexec(pchar(cmd) , sw_hide);
      end;
      ...
      if ( pos( 'S',  funcoes.lerParam('upgrade.ini',03) ) > 0 ) then
      begin
         MostraMensagem(sender, 'Ajustando data e hora',300);
         dt.Host := Funcoes.lerParamNome('ConexaoaoWell.ini', 'data Source');
...
begin
   if ( funcoes.lerParam('upgrade.ini',0)) = '' then
   begin
   ...
   begin
      ...
      if (pos('walter', GetNomeDoMicro()) >  0) then
      begin
         if MsgTela('',' Atualizar os programas ?', MB_YESNO+ MB_ICONQUESTION) = mrYes then
            Atualizar(Sender)
         else
         begin
            ExecutarAplicativo( ProgramaAExecutar );
