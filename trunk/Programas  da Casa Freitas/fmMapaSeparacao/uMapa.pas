...
uses uCriaMapa, uMain, fmAbrirAvarias, cf;
...
var
   ...
   numMapa,cmd:String;
begin
   dsItem := cf.getDadosProd(fmMain.getUoLogada(), codigo, '', '5', true);
...
begin
   if (alterada = true )then
      if funcoes.msgTela('','Foram feitas alterações no mapa, deseja salva-las? ', MB_YESNO ) = mrYes then
         Salvar1Click(Sender);
   ...
   begin
      ...
      begin
         ...
         begin
          ...
          begin
//             cmd := funcsql.gerarRequisicao( fmMain.Conexao, tbReq, uos[i], autorizador, false, false, ocoItens, QT_DIAS_PEND);
             cmd :=   cf.gerarRequisicao( tbReq, uos[i], fmMain.getUOCD(), autorizador, false, false, ocoItens, QT_DIAS_PEND);
             lj:= lj + cmd +'   ';
          end;
      end;
       nTbReq := nTbReq + lj + #13;
         ...
         begin
            ...
            begin
              aut := 'update zcf_mapaSeparacao set ehFinalizada = 1 , aprovador = '+aut+ ' , historico = ' + funcoes.getLinhasMemo(mmObs.Lines) +
                     ' , requisicoes = ' + quotedStr( geraRequisicoes(aut) ) +  ' where num = ' + lbNumMp.Caption;
