...
begin
   ...
   fmMain.getListaLojas(cbLojaEstoque, true,false, '');
...
begin
   cmd :=  ' Select ( Select top 01 dt_movpd from dspdi with(nolock) where  dspdi.is_planod =  dspd.is_planod )  as Data , ' +#13
           ...
           +' zcf_tbuo.ds_uo as Loja, dsusu.nm_usu '       +#13
           +' from dspdi with(NoLock) ' +#13
           +' inner join dspd  with(NoLock) on dspd.is_planod = dspdi.is_planod ' +#13
           +' inner join zcf_tbuo  with(NoLock) on dspdi.is_estoque = zcf_tbuo.is_uo ' +#13
           ...
           cmd := cmd
           +' group by dspd.is_planod, dt_planod, zcf_tbuo.ds_uo, dspdi.is_estoque, dsusu.nm_usu ' +#13
