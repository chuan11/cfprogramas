...
begin
  ...
  if ( cbDataAdmissao.Checked = true) then
   ...
   ds := funcsql.getDataSetQ(cmd, Form2.Conexao );
   ds.First();
...
begin
  ...
  if ( cbDataAdmissao.Checked = true) then
   ...
   while (ds.Eof = false) do
   begin
      if (uUtil.insereRegistroFerias('F', ds.fieldByName('rel_matricula').AsString, ds.fieldByName('rel_dataRef1').AsDateTime, ds.fieldByName('rel_dataref2').AsDateTime) = true) then
         inc(qtInseridos);
      ds.Next();
   end;
   ds.Close();
...
begin
   ...
   funcsql.getTable(form2.Conexao, tbEmp, 'base int, estabelecimento int, matricula varchar(08), cartaoPonto varchar(08), nome varchar(50), funcao varchar(30), dataAdmissao smallDateTime ');
   ...
   funcsql.execSQL(cmd, Form2.Conexao);
   tbEmp.Close;
   tbEmp.Open();
