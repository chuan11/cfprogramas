...
interface
   ...
   procedure calculaRRANA(is_refs:TStringlist; uo:String; dti, dtf:Tdate);
...
begin
   ...
   if ( ds.IsEmpty = true) then
      ...
      result :=  floatTostrf(  funcSQL.somaColunaTable(ds,'qt_mov') , ffgeneral, 18, 0);
end;
...
begin
   cmd:= ' select '+#13+
         ...
         ' dlest.is_estoque= ' + uo +#13+
         ' and dlest.is_ref = ' + is_ref  +#13+
         ' and dt_mov between '+ funcDatas.dateToSqlDate(dti) + ' and ' + funcDatas.dateToSqlDate(dti) + #13+
         ' and dlest.is_tpcte= 6 ' +#13+
         ' and st_mov = '''' ' +#13+
...
begin
   ...
   cmd := ' is_ref int, cd_ref varchar(08), ds_ref varchar(50), saldoAnterior int';
   funcsql.getTable(fmMain.Conexao, tbItens, cmd);
//   tbItens.tableName:= funcSQL.criaTabelaTemporaria(fmMain.conexao, cmd);

   tbMov := TADOTable.create(nil);
   cmd := 'dt_mov smallDateTime, ds_res varchar(50), nm_usu varchar(50), qt_mov int, qt_sldatu int, ' +
          'TipoMovEstoque varchar(1), is_fildest varchar(08), ds_uo varchar(50)';
   tbMov.tableName:= funcSQL.criaTabelaTemporaria(fmMain.conexao, cmd);


   for i:= 0 to is_refs.count -1 do
   begin
      ds := uCF.getDadosProd(uo, is_refs[i], '101', false);
      ...
      ds:= uCF.getRRANAItem(is_refs[i], uo, dti, dtf);
      if (ds.isEmpty = false) then
      begin
